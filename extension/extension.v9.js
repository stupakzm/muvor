...
export default class MuvorExtension extends Extension {
     ...
     *  is the mode staying the mode. Re-asserted from an idle because
     *  setting the focus from inside `key-focus-out` is re-entrant. */
    _onKeyFocusOut() {
        if (this._state !== 'keys' || !this._overlay || this._refocusId)
            return;
    ...
    _enterKeyGrab() {
         ...
         * deadman, and the same argument (§5.1b). */
        if (this._deadman)
            GLib.source_remove(this._deadman);
